Shader "Custom/PlayerOcclusion_bark" {
	Properties{
		...
		SubShader{
			...
			// variables du shader pour faire disparaitre les objets
			half _Distance = 5; // largeur du cercle (correspond a width)
			int _ObjectsLength = 0; // nombre d'objets a tester
			fixed4 _Objects[16];
			float3 viewDir;
			...
			void surf(Input IN, inout SurfaceOutputStandard o) {
				...
				float outalpha = 1; // set alpha to alpha max on pixel
